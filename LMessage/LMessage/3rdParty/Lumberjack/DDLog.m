...
#endif
#define KLogZipPassword @"wondershare"
...
+ (NSString *)getLogText{
    ...
    for (DDLoggerNode *loggerNode in loggers) {
        DDFileLogger *fileloger = (DDFileLogger *)[loggerNode loger];
        if ([fileloger isKindOfClass:[DDFileLogger class]]) {
            NSArray *files = [fileloger.logFileManager sortedLogFilePaths];
            for (NSString *file in files) {
                [result appendString:[NSString stringWithContentsOfFile:file encoding:NSUTF8StringEncoding error:nil]];
            }
        }
    }
    return result;
...
+ (NSString*)getLogContentWith:(NSString*)url{
    NSMutableString *result = [NSMutableString string];
    return result;
}

+ (NSString *)compressToZip{//add by shizejing
    NSString *result = nil;
    return [result stringByAppendingString:@"/log.zip"];
}
...
+ (NSString *)getLogDirectory{
    ...
    for (DDLoggerNode *loggerNode in loggers) {
        DDFileLogger *fileloger = (DDFileLogger *)[loggerNode loger];
        if ([fileloger isKindOfClass:[DDFileLogger class]]) {
            result = [fileloger.logFileManager logsDirectory];
            break;
        }
    }
    return result;
...
- (id <DDLogger>)loger{return logger;}
